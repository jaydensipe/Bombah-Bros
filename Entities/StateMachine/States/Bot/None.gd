...
func enter(_msg := {}) -> void:
	if (_msg.has("Reload")):
		bot.wait_for_reload_timer.start()
		if(!bot.wait_for_reload_timer.timeout.is_connected(_switch_to_reload)):
			bot.wait_for_reload_timer.timeout.connect(_switch_to_reload)
